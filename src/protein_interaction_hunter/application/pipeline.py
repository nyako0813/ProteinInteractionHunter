"""MVP-1A orchestration through candidate generation only."""
...
class InteractionCandidatePipeline:
    """Generate auditable candidates without running evidence or scoring engines."""
    ...
    def run(
        ...
        if config_path is None:
            raise NotImplementedError(
                "Candidate ranking is not implemented in MVP-0; MVP-1A candidate generation "
                "requires a config path."
            )
        resolved_config_path = config_path.expanduser().resolve()
        config = load_config(resolved_config_path)
        proteins = LocalFastaLoader().load(config.input.proteome_fasta)
        coordinates = LocalGff3Loader().load(config.input.genome_gff)
        ...
        candidate_table_path = CandidateTableTsvWriter().write(
            run_id, generated.candidates, output_path / "candidate_table.tsv"
        )
        all_warnings = [
            warning for candidate in generated.candidates for warning in candidate.warnings
        ]
