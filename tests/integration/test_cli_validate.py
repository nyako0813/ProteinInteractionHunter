...
def test_cli_validate_config(valid_config_path: Path) -> None:
    result = runner.invoke(
        app, ["validate-config", "--config", str(valid_config_path)]
    )
    assert result.exit_code == 0
...
def test_cli_validate_inputs(valid_config_path: Path) -> None:
    result = runner.invoke(
        app, ["validate-inputs", "--config", str(valid_config_path)]
    )
    assert result.exit_code == 0
    assert "proteins=12" in result.stdout
    assert "duplicates=1" in result.stdout
    assert "missing_coordinates=1" in result.stdout
...
def test_cli_inspect_fixture_has_counts_but_no_ranking(
    ...
    result = runner.invoke(
        app, ["inspect-fixture", "--config", str(valid_config_path)]
    )
    assert result.exit_code == 0
